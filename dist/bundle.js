...
(function () {
	...
	var recursiveMask = '#.##';
	recursiveMask.split('').map(function (token) {
	    if (tokens[token]) {
	        return tokens[token].pattern;
	    }
	    return token;
	}).join('');
	var continueCondition = function (current, end, avoidInfiniteLoop) {
	    if (avoidInfiniteLoop > 20)
	        return false;
	    return current < end;
	};
	var maskify = function (string, mask) {
	    ...
	    while (continueCondition(maskPosition, end, avoidInfiniteLoop)) {
	        var stringChar = string.charAt(stringPosition); //a
	        var patternChar = mask.charAt(maskPosition); //9
	        var token = tokens[patternChar];
	        console.log('while', {
	     ...
	     * Loop over the mask
	     */
	    for (var maskPosition_1 = start; maskPosition_1 < end; maskPosition_1++) {
	        var stringChar = string.charAt(stringPosition);
	        var patternChar = mask.charAt(maskPosition_1);
	        var token = tokens[patternChar];
	    }
	};
	...
	var App = function () {
	    ...
	    var _a = react.exports.useState('1234'), value = _a[0], setValue = _a[1];
	    var mask = '##//SS/####';
